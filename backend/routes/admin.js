...
// =============================================================
// System health — JWT secret strength, DB pulse, OTP delivery pulse.
// Used by the Bots Control page header to flag deployment issues
// (weak JWT, no recent OTPs across all sources, etc.).
// =============================================================
router.get('/system/health', (req, res) => {
  ...
  try {
    ...
    let length = 0;
    if (fromEnv && fromEnv.length >= 32) { source = 'env'; length = fromEnv.length; }
    else {
      const stored = db.prepare("SELECT value FROM settings WHERE key='jwt_secret'").get()?.value || '';
      if (stored) { source = 'settings'; length = stored.length; }
    }
    out.jwt = { source, length, strong: length >= 32 };
  } catch (e) { out.jwt = { error: e.message }; }

  // Last CDR row across the whole platform — any bot delivering OTPs
  try {
    const lastReal = db.prepare(`
      SELECT created_at FROM cdr
      WHERE COALESCE(note,'') NOT LIKE 'fake:%'
      ORDER BY id DESC LIMIT 1
    `).get();
    const lastAny = db.prepare(`SELECT created_at FROM cdr ORDER BY id DESC LIMIT 1`).get();
    out.cdr = {
      last_real_at: lastReal?.created_at || null,
      last_any_at:  lastAny?.created_at  || null,
      total_today:  db.prepare(`
        SELECT COUNT(*) AS n FROM cdr
        WHERE created_at >= strftime('%s', 'now', 'start of day')
          AND COALESCE(note,'') NOT LIKE 'fake:%'
      `).get()?.n || 0,
  ...
  // Active allocations awaiting OTP — pressure metric for portals
  try {
    out.allocations = {
      active: db.prepare(`SELECT COUNT(*) AS n FROM allocations WHERE status='active'`).get()?.n || 0,
    };
  } catch (e) { out.allocations = { error: e.message }; }
