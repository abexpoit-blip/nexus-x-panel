...
const IMSHealth = () => {
  ...
  const { data, isLoading, refetch, dataUpdatedAt } = useQuery({
    queryKey: ["admin-bots-ims"],
    queryFn: () => api.admin.bots.list(),
    refetchInterval: 5_000,
  });

  const { data: logs } = useQuery({
    queryKey: ["admin-bots-ims-logs"],
    queryFn: () => api.admin.bots.logs("ims", "all", 60),
    refetchInterval: 8_000,
  });

  const ims = data?.bots?.ims;
  ...
  const action = async (a: "start" | "stop" | "restart") => {
    setBusy(a);
    try {
      await api.admin.bots.action("ims", a);
      toast({ title: `${a} sent`, description: "IMS bot" });
      setTimeout(() => qc.invalidateQueries({ queryKey: ["admin-bots-ims"] }), 700);
    } catch (e) {
  ...
  const probe = async () => {
    setBusy("health");
    try {
      const r = await api.admin.bots.health("ims");
  ...
  if (isLoading) {
    return (
      <div className="p-12 text-center text-muted-foreground">
        <Loader2 className="w-6 h-6 animate-spin mx-auto mb-2" /> Loading IMS health…
      </div>
    );
  }
  ...
  if (!ims) {
    return (
      <GlassCard className="!p-8 text-center text-muted-foreground">
        IMS bot module not detected on backend.
      </GlassCard>
    );
  }
  ...
  return (
    <div className="relative space-y-6">
      ...
      <PageHeader
        eyebrow="Diagnostics"
        title="IMS Bot — Health & Debug"
        description="Live cooldown, rate-limit telemetry, session state and recent errors for the IMS scraper."
